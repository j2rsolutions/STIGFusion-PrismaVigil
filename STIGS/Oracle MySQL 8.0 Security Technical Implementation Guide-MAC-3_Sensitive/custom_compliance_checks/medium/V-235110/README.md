# V-235110 - The MySQL Database Server 8.0 must generate audit records when unsuccessful attempts to access categories of information (e.g., classification levels/security levels) occur.
...
**Fix Text**:
```If currently required, configure the MySQL Database Server with views that use selects that call audit\_api\_message\_emit\_udf\(\) function to produce audit records when selection of categories of information occurs\.
